...
# Installs to /opt/koulur with a /usr/bin/koulur symlink, matching the
...
cat > "$TOPDIR/SPECS/koulur.spec" <<'EOF'
Name: koulur
Version: %{pkg_version}
Release: %{pkg_release}%{?dist}
Summary: Koulur colour palette generator
License: AGPL-3.0-only
URL: https://gitlab.com/HttpAnimations/koulur
...
%global __provides_exclude_from ^/opt/koulur/.*

%description
A colour palette generator: harmonious five-colour palettes, lockable
swatches and one-tap hex copy.

%install
mkdir -p %{buildroot}/opt/koulur %{buildroot}/usr/bin
cp -a %{bundle_dir}/. %{buildroot}/opt/koulur/
ln -sf /opt/koulur/koulur %{buildroot}/usr/bin/koulur
