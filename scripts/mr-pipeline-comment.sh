...
PROJECT_PATH="${CI_PROJECT_PATH:-HttpAnimations/koulur}"
MARKER="<!-- mr-pipeline-${PIPELINE_ID} -->"
PIPELINE_URL="${SERVER_URL}/${PROJECT_PATH}/-/pipelines/${PIPELINE_ID}"
RUN_URL="${RUN_ID:+https://github.com/justacalico/koulur/actions/runs/${RUN_ID}}"
...
GitHub run: [View on GitHub](${RUN_URL})"
  ...
  finish)
    ...
    if [ -n "$RUN_ID" ] && command -v gh >/dev/null 2>&1; then
      jobs=$(gh run view "$RUN_ID" -R justacalico/koulur --json jobs 2>/dev/null | jq -r '.jobs[] | select(.conclusion != null or .status != null) | "- **\(.name)**: \(.conclusion // .status)"' || true)
      downloads=$(gh api "repos/justacalico/koulur/actions/runs/${RUN_ID}/artifacts?per_page=100" 2>/dev/null \
        | jq -r --arg run "$RUN_ID" '.artifacts[] | select(.expired != true) | "- [\(.name)](https://github.com/justacalico/koulur/actions/runs/\($run)/artifacts/\(.id))"' || true)
    fi
